...
support eight types of rotation that are pre-defined in ({\tt inside}, {\tt outside}, {\tt reverse.clockwise}, 
{\tt clockwise}, {\tt downward}, {\tt bending.inside}, {\tt bending.outside}).
But {\tt adj} argument is still applicable in {\tt circos.text}. Please note for {\tt bending.inside}
and {\tt bending.outside}, currently, single line text is only supported.
...
{\tt outside}, {\tt clockwise}, {\tt reverse.clockwise}, {\tt bending.inside} and {\tt bending.outside}. When {\tt niceFacing} is on, values for internal {\tt facing}
...
examples to highlight sectors and tracks (figure \ref{fig:circlize_highlight} A).
...
If the purpose is simply highlight complete cells, there is a shortcut function
{\tt code highlight.sector} for which you only need to specify index for sectors
and tracks that you want to to highlight (figure \ref{fig:circlize_highlight} B).
